...
export default function AuthorPage() {
  ...
  return (
    <div className="fade-in-up">
      ...
      {loading ? (
        <p style={{ color: 'var(--muted)', fontStyle: 'italic', fontSize: '0.9rem' }}>Loading author...</p>
      ) : notFound || !data ? (
        <p style={{ color: 'var(--muted)', fontStyle: 'italic', fontSize: '0.9rem' }}>Author not found.</p>
      ) : (
        <>
          <header style={{ marginBottom: '2.5rem' }}>
            <h1 style={{ marginBottom: '0.25rem' }}>{data.name}</h1>
            <p style={{ color: 'var(--muted)', fontSize: '0.9rem', letterSpacing: '0.02em' }}>
              {data.works.length} {data.works.length === 1 ? "book" : "books"} in your library
            </p>
          </header>
          ...
          <DiscoverSection
            title={`More by ${data.name}`}
            emptyText={`No other ${data.name} books found that aren't already in your library.`}
            fetchUrl={`${process.env.NEXT_PUBLIC_API_URL}/authors/${data.id}/discover`}
            linkAuthorId={data.id}
            onAdded={fetchAuthor}
          />
        </>
      )}
